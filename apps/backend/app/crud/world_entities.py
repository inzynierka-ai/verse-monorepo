...
def get_entity_names_by_story_id(db: Session, story_id: int) -> List[str]:
    entities = db.query(WorldEntityModel).filter(
        WorldEntityModel.story_id == story_id).all()
...
def get_related_entities(db: Session, query_embedding: List[float], story_id: int,
    ...
    try:
        ...
        stmt = (
            select(WorldEntityModel, (1 -
                   WorldEntityModel.embedding.cosine_distance(query_embedding)).label("similarity"))
        ...
        for entity, similarity in results:
            logging.info(
                f"Found related entity: {entity.name} with similarity score: {similarity:.4f}")
...
def get_related_entities_by_name(db: Session, entity_name: str, story_id: int,
    ...
    try:
        ...
        if not query_embedding:
            logging.error(
                f"Failed to generate embedding for entity name: {entity_name}")
            return []

        logging.info(
            f"Generated embedding for '{entity_name}' with length {len(query_embedding)}")
...
def get_entities_by_name(db: Session, query: str, story_id: int, search_descriptions: bool = False):
    ...
    try:
        ...
        base_query = db.query(WorldEntityModel).filter(
        ...
        if search_descriptions:
            ...
            base_query = base_query.filter(
                or_(name_condition, alias_condition, desc_condition))

        # Execute the query
        entities = base_query.all()

        logging.info(
            f"Search for '{query}' in {log_source} found {len(entities)} entities")
        return entities
...
def save_entity(db: Session, entity_data: WorldEntityFromLLM, story_id: int,
    ...
    try:
        ...
        if hasattr(entity_data, 'aliases') and entity_data.aliases:
            ...
            logging.info(
                f"Entity '{entity_data.name}' has {len(aliases)} aliases: {aliases}")
        else:
        ...
        logging.info(
            f"Saved new world entity: {entity_data.name} (ID: {db_entity.id}) at {current_time}")
        return db_entity.id
