...
class StoryGenerator:
    ...
    async def generate_story(self, user_id: int, story_gen_input: StoryGenerationInput) -> Story:
        ...
        if violated_categories:
            ...
            langfuse_context.update_current_trace(
                metadata={"violated_categories": violated_categories})
            langfuse_context.update_current_trace(
                output={"ERROR": "Violated categories"})
            raise ValueError(
                f"Sorry, we can't generate your story because it contains content that is not allowed. Please try again with different description. Reason: {', '.join(violated_categories.keys())}")
    ...
    async def extract_story_entities(self, story: Story) -> List[int]:
        ...
        try:
            ...
            logging.info(
                f"Extracted {len(entity_ids)} world entities from story {story.title}")
            return entity_ids
    ...
    async def _generate_story_details(self, description: str, story_gen_input: StoryGenerationInput) -> StoryDetails:
        ...
        messages = [
            ...
            self.llm_service.create_message(
                "user", CREATE_STORY_DETAILS_JSON_USER_PROMPT)
        ]
        ...
        story_details = JSONService.parse_and_validate_json_response(
