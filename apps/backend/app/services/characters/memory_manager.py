...
class MemoryManager:
    ...
    async def create_memories(self, db_session: Session, scene_uuid: uuid.UUID) -> List[Any]:
        ...
        try:
            ...
            print(
                f"DEBUG: Found characters: {[c.name for c in characters] if characters else 'None'}")
            if not characters:
                return []
            ...
            for character in characters:
                try:
                    logging.info(
                        f"Processing character {character.name} with UUID {character.uuid}")

                    memory_chunks = self.create_memory_chunks(scene, character)
                    logging.info(
                        f"Created {len(memory_chunks)} memory chunks for character {character.name}")
                    ...
                    for memory_chunk in memory_chunks:
                        ...
                        if memory_id:
                            ...
                            logging.warning(
                                f"Failed to save memory chunk for character {character.name}")
                except Exception as e:
                    logging.error(
                        f"Error processing character {character.name}: {str(e)}")
                    traceback.print_exc()

            logging.info(
                f"Memory creation completed, created {len(memories_created)} memories")
    ...
    def create_memory_chunks(self, scene: Scene, character: Character) -> List[Dict[str, Any]]:
        ...
        try:
            logging.info(
                f"Creating overlapping memory chunks for scene {scene.uuid} and character {character.uuid}")

            if not self.db_session:
                logging.error(
                    "No database session available, cannot create memory chunks")
                return []

            messages = get_messages_by_scene_and_character(
                self.db_session, scene.uuid, character.uuid)
            if not messages:
                logging.warning(
                    f"No conversation found for scene {scene.uuid} and character {character.uuid}.")
                return []

            # Need at least 3 messages to create a chunk
            if len(messages) < 3:
                logging.warning(
                    f"Not enough messages ({len(messages)}) to create overlapping chunks")
            ...
            logging.info(
                f"Created {len(memory_chunks)} overlapping memory chunks from {len(messages)} messages")
    ...
    async def find_similar_memories(self, character_id: int, query: str, top_n: int = 3,
        ...
        try:
            logging.info(
                f"Finding similar memories for character {character_id}, query: '{query[:50]}...'")

            if not self.db_session:
                logging.error(
                    "No database session available, cannot find similar memories")
                return []
            ...
            if not memories:
                logging.info(
                    f"No similar memories found for character {character_id} above threshold {similarity_threshold}")
                return []

            logging.info(
                f"Found {len(memories)} similar memories with similarity >= {similarity_threshold}")
            ...
            for memory in memories:
                if hasattr(memory, 'embedding'):
                    ...
                    similarity = cosine_similarity(
                        query_embedding_np, memory_embedding_np)[0][0]
                    logging.info(
                        f"Memory ID: {memory.uuid}, Similarity: {similarity:.4f}, Text: '{memory.memory_text[:50]}...'")
