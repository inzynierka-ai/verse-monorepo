...
class ModelConverter:
    ...
    def orm_to_pydantic(
        ...
        Args:
            from_attributesl: The database ORM model instance
        ...
        try:
            ...
            if hasattr(from_attributesl, "__dict__"):
                orm_dict = from_attributesl.__dict__.copy()
                # Remove SQLAlchemy internal attributes
                orm_dict.pop("_sa_instance_state", None)
            else:
                orm_dict = dict(from_attributesl)
        ...
        try:
            ...
            logger.error(
                f"Failed to validate {pydantic_model.__name__} from ORM data: {e}")
            raise ValueError(
                f"Failed to convert to {pydantic_model.__name__}: {str(e)}")
    ...
    def character_orm_to_pydantic(
        ...
        if not getattr(character_orm, "goals", None):
            ...
            defaults["goals"] = [goal.strip()
                                 for goal in goals_str.split(",") if goal.strip()]
        ...
        try:
            ...
            logger.error(
                f"Character ORM data: {character_orm.__dict__ if hasattr(character_orm, '__dict__') else character_orm}")
    ...
    def location_orm_to_pydantic(
        ...
        if not getattr(location_orm, "rules", None):
            ...
            defaults["rules"] = [rule.strip()
                                 for rule in rules_str.split(",") if rule.strip()]
        ...
        try:
            ...
            logger.error(
                f"Location ORM data: {location_orm.__dict__ if hasattr(location_orm, '__dict__') else location_orm}")
            logger.error(f"Applied defaults: {defaults}")

            # Fall back to manual creation with minimal required fields
            return LocationSchema(
                name=getattr(location_orm, "name", "Unknown Location"),
                description=getattr(location_orm, "description",
...
def convert_characters(
    ...
    for character_orm in character_orms:
        try:
            ...
            logger.error(
                f"Failed to convert character {getattr(character_orm, 'name', 'unknown')}: {e}")
...
def convert_locations(
    ...
    for location_orm in location_orms:
        try:
            ...
            logger.error(
                f"Failed to convert location {getattr(location_orm, 'name', 'unknown')}: {e}")
...
def convert_scenes(
    ...
    for scene_orm in scene_orms:
        try:
            ...
            logger.error(
                f"Scene ORM data: {scene_orm.__dict__ if hasattr(scene_orm, '__dict__') else scene_orm}")
            raise e
